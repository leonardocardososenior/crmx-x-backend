...
/**
 * Comprehensive system validation for business proposals
 */
export async function validateSystemIntegrity(): Promise<ValidationResult> {
  const result: ValidationResult = {
    isValid: true,
    errors: [],
    warnings: []
  ...
  try {
    // 1. Check for orphaned proposal items
    const { data: orphanedItems } = await supabaseAdmin
      .from('business_proposal_item')
      .select('id, proposal_id')
      .not('proposal_id', 'in', 
        `(${await supabaseAdmin.from('business_proposal').select('id').then(r => r.data?.map(p => `'${p.id}'`).join(',') || "''")})`
      );

    if (orphanedItems && orphanedItems.length > 0) {
      result.warnings.push(`Found ${orphanedItems.length} orphaned proposal items`);
    }

    // 2. Check for proposals with invalid business references
    const { data: invalidBusinessRefs } = await supabaseAdmin
      .from('business_proposal')
      .select('id, business_id')
      .not('business_id', 'in',
        `(${await supabaseAdmin.from('business').select('id').then(r => r.data?.map(b => `'${b.id}'`).join(',') || "''")})`
      );

    if (invalidBusinessRefs && invalidBusinessRefs.length > 0) {
      result.errors.push(`Found ${invalidBusinessRefs.length} proposals with invalid business references`);
      result.isValid = false;
    }

    // 3. Check for proposals with invalid user references
    const { data: invalidUserRefs } = await supabaseAdmin
      .from('business_proposal')
      .select('id, responsible_id')
      .not('responsible_id', 'in',
        `(${await supabaseAdmin.from('users').select('id').then(r => r.data?.map(u => `'${u.id}'`).join(',') || "''")})`
      );

    if (invalidUserRefs && invalidUserRefs.length > 0) {
      result.errors.push(`Found ${invalidUserRefs.length} proposals with invalid user references`);
      result.isValid = false;
    }

    // 4. Check for calculation inconsistencies
    const { data: proposals } = await supabaseAdmin
      .from('business_proposal')
      .select(`
        id, 
        value,
        business_proposal_item(total)
      `);

    if (proposals) {
      for (const proposal of proposals) {
        const items = (proposal as any).business_proposal_item || [];
        const itemsTotal = items.reduce((sum: number, item: any) => sum + item.total, 0);
        
        if (Math.abs((proposal as any).value - itemsTotal) > 0.01) {
          result.warnings.push(`Proposal ${proposal.id}: value mismatch (${(proposal as any).value} vs ${itemsTotal})`);
        }
      }
    }

  } catch (error) {
    logger.error('VALIDATION', 'Error during system integrity validation', error as Error);
    result.errors.push('Internal system validation error');
    result.isValid = false;
  }
